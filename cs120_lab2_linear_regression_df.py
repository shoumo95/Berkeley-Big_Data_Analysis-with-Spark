# Databricks notebook source exported at Sat, 22 Oct 2016 22:47:14 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    """
    interDf = df.select(sql_functions.split(df['value'], ','))
    labelDf = interDf.map(lambda rowLst: LabeledPoint(float(rowLst[0][0]), [float(x) for x in rowLst[0][1:]])).toDF()
    return labelDf
