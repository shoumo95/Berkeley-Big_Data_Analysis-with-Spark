# Databricks notebook source exported at Sat, 22 Oct 2016 22:47:46 UTC
# MAGIC %md
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    return SparseVector(num_ohe_feats, [(ohe_dict_broadcast.value[featureTpl], 1.) for featureTpl in raw_feats])
...
def parse_point(point):
    ...
    return [(idx - 1, parsedstr[idx]) for idx in xrange(1, len(parsedstr))]
...
def parse_raw_df(raw_df):
    ...
    """
    newDf = raw_df.select(split('text', ",")[0].cast("double").alias('label'), parse_point_udf('text').alias("feature"))
    return newDf.cache()
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    Returns:
        SparseVector: A SparseVector of length num_ohe_feats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(num_ohe_feats, [(ohe_dict_broadcast.value[ftrTpl], 1.) for ftrTpl in raw_feats \
...
def add_log_loss(df):
    ...
    """
    return df.select("*", when(df.label == 1, 0. - log(df.p + epsilon)).otherwise(0. - log(1. - df.p + epsilon)).alias('log_loss'))
